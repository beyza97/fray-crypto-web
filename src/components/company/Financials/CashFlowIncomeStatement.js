...
export const CashFlowIncomeStatement = () => {
    ...
    useEffect(() => {
        if (code)
            api.get(`/company/${code}/Nakit Akış Tablosu (Dolaylı Yöntem)`).then(res => {
    ...
    const saveAsExcelFile = (buffer) => {
        import('file-saver').then(module => {
            if (module && module.default) {
                ...
                module.default.saveAs(data, code + ' Nakit Akım Detayı');
            }
        });
    }
